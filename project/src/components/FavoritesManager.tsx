...
import { Heart, Star, BookOpen, Search, X } from 'lucide-react';
...
export function FavoritesManager({ onFavoriteClick, direction }: FavoritesManagerProps) {
  const { favorites, removeFavorite, clearFavorites, getFavoritesCount } = useFavorites();
        ...
        <div className="mt-3 sm:mt-8 text-center">
          <div className="inline-flex items-center space-x-1 sm:space-x-2 bg-gradient-to-r from-red-50 to-rose-50 px-3 sm:px-6 py-1.5 sm:py-3 rounded-full border border-red-200">
            ...
            <span className="text-red-700 font-medium text-xs sm:text-base">
              {favoritesCount} {favoritesCount === 1 ? 'слово' : favoritesCount < 5 ? 'слова' : 'слов'} в избранном
            </span>
          </div>
        </div>
      </div>

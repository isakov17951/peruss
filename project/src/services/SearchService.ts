...
interface WordData {
  ...
  translation?: string;
  translations?: Array<{ definition: string }>;
  examples?: Array<{ example_text: string }>;
  rus_examples?: Array<{ example_text: string }>;
  pers_examples?: Array<{ example_text: string }>;
}
...
export class SearchService {
   ...
   */
  static formatSearchResult(wordData: WordData): Translation {
    console.log('🔄 SearchService: Formatting result:', wordData);

    // Собираем все примеры
    const allExamples = [
      ...(wordData.examples?.map(ex => ex.example_text) || []),
      ...(wordData.rus_examples?.map(ex => ex.example_text) || []),
      ...(wordData.pers_examples?.map(ex => ex.example_text) || [])
    ...
    if (wordData.translations && wordData.translations.length > 0) {
      const translationDefs = wordData.translations.map(t => t.definition);

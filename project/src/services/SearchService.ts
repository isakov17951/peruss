...
export class SearchService {
   ...
   */
  static formatSearchResult(wordData: any): Translation {
    console.log('🔄 SearchService: Formatting result:', wordData);

    // Собираем все примеры
    const allExamples = [
      ...(wordData.examples?.map((ex: any) => ex.example_text) || []),
      ...(wordData.rus_examples?.map((ex: any) => ex.example_text) || []),
      ...(wordData.pers_examples?.map((ex: any) => ex.example_text) || [])
    ...
    if (wordData.translations && wordData.translations.length > 0) {
      const translationDefs = wordData.translations.map((t: any) => t.definition);
   ...
   */
  static validateSearchQuery(query: string): Result<string> {
